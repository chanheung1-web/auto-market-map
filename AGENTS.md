...
## サプライチェーン（構造）と バリューチェーン（値動き）は別物

画面に2つの一覧がある。混同しないこと。

| セクション | 何を見る | 出どころ |
|---|---|---|
| サプライチェーン | **誰がどれだけ握っているか**（市場規模・CAGR・上位5社シェア） | `supplyChain.ts`（静的） |
| バリューチェーン | **今日いくら動いたか**（監視129銘柄の株価） | Yahoo（60秒ごと） |

構造を先に置いているのは、値動きの意味が構造を知らないと読めないため。
「SiCが3社で67%」を知って初めて「STMicroが下げた日」の重さが分かる。

### supplyChain.ts の出どころと制約

OneDrive の `automotive_supply_chain_v2.xlsx`（2026-04-22作成、**2024年基準**）を
変換した静的スナップショット。15カテゴリ×上位5社＝75レコード。

- シェアは Mordor Intelligence 等の**調査会社推計（±数%）**。断定に使わない
- **カテゴリ間で金額を足さないこと。**調査会社ごとに市場の定義が違う
  （ai-datacenter-tracker の `marketSegments.ts` と同じ注意）
- 数字を直すときは Excel ではなく**このファイルを直す**。Excel はもう更新されない
- 元データに**電池と完成車のカテゴリが無い**。階層タブで「電池・EV基幹」「完成車」を
  選ぶと0件になるのはこのため（バグではない）

`companyId` は `companies.ts` への紐付けで、75社中34社が繋がっている。
null は非上場か、このアプリが株価を追っていない企業。青い社名だけ押せて、
押すとニュースが絞られる。表記ゆれは変換時に手当てした（"Denso Corporation"→
デンソー、"Fuyao Glass (福耀玻璃)"→福耀ガラス 等）。
